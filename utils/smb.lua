...
function smb1rng_advance(seed, count)
    if count == nil then
        count = 1
    end
    for i=0,count,1 do
        seed = smb1rng_step(seed)
    end
    return seed
end

function smb1rng_init()
    return { 0xA5, 0, 0, 0, 0, 0, 0, 0 }
end

function smb1rng_apply(seed)
    for i=1,8,1 do
        memory.writebyte(0x7A6 + i, seed[i])
    end
end

function smb1rng_read()
    local result = {}
    for i=1,8,1 do
...
function smb1_readscore()
    local result = 0
    for i=1,6,1 do
        result = result + ((10 ^ (6 - i)) * memory.readbyte(PlayerScoreDisplay + i))
    end
    return result
end